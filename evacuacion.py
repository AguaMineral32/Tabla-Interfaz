...
def crear_tabla_evacuacion(archivo):
    try:
        tipos_operatividad = ['EVACUACION_TOTAL', 'EVACUACION_PARCIAL', 'NO_EVACUADO']
        df_operativos = archivo[archivo['ESTADO_EVACUACION'].isin(tipos_operatividad)]

        # Generar la tabla resumen
        conteo_establecimientos = (
            df_operativos.groupby(['TIPO_ESTABLECIMIENTO', 'ESTADO_EVACUACION'])
            .size()
        ...
        conteo_establecimientos = conteo_establecimientos[['EVACUACION_TOTAL', 'EVACUACION_PARCIAL', 'NO_EVACUADO']]
        ...
        tablas_Operatividad['principalESTADO_EVACUACION'] = conteo_establecimientos_html
        ...
        for operatividad in tipos_operatividad:
            for tipo_establecimiento in archivo['TIPO_ESTABLECIMIENTO'].unique():
                ...
                df_filtrado = archivo[(archivo['TIPO_ESTABLECIMIENTO'] == tipo_establecimiento) & 
                                (archivo['ESTADO_EVACUACION'] == operatividad)]
                
                # Si el DataFrame no está vacío, proceder
                if not df_filtrado.empty:
                    columnas = ['NOMBRE_ESTABLECIMIENTO', 'ESTADO_EVACUACION','HORA_EDAN','FECHA_EDAN']
